...
namespace HospitalAppoimentSolution.Helper.Core
{
    public class VPCRequest_VN
    {
        Uri _address;
        ...
        public VPCRequest_VN(String URL)
        {
    ...
    public class ErrorLocalPayment
    {
        public int Code { get; set; }
        public string Message { get; set; }
    }

    public class CreateErrorLocal
    {
        public List<ErrorLocalPayment> errorLocals()
        {
            List<ErrorLocalPayment> errorLocalPayments = new List<ErrorLocalPayment>();
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 0, Message = "Giao dịch thành công." });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 1, Message = "Giao dịch không thành công, Ngân hàng từ chối giao dịch." });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 3, Message = "Giao dịch không thành công, Mã đơn vị không tồn tại" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 4, Message = "Giao dịch không thành công, Không đúng access code" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 5, Message = "Giao dịch không thành công, Số tiền không hợp lệ" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 6, Message = "Giao dịch không thành công, Mã tiền tệ không tồn tại" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 7, Message = "Giao dịch không thành công, Lỗi không xác định" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 8, Message = "Giao dịch không thành công, Số thẻ không đúng " });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 9, Message = "Giao dịch không thành công, Tên chủ thẻ không đúng" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 10, Message = "Giao dịch không thành công, Thẻ hết hạn/Thẻ bị khóa" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 11, Message = "Giao dịch không thành công, Thẻ chưa đăng ký sử dụng dịch vụ" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 12, Message = "Giao dịch không thành công, Ngày phát hành/Hết hạn không đúng" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 13, Message = "Giao dịch không thành công, Vượt quá hạn mức thanh toán" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 21, Message = "Giao dịch không thành công, Số tiền không đủ để thanh toán" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 22, Message = "Giao dịch không thành công, Thông tin tài khoản không đúng" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 23, Message = "Giao dịch không thành công, Tài khoản bị khóa" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 24, Message = "Giao dịch không thành công, Thông tin thẻ không đúng" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 25, Message = "Giao dịch không thành công, OTP không đúng." });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 253, Message = "Giao dịch không thành công, Quá thời gian thanh toán" });
            errorLocalPayments.Add(new ErrorLocalPayment { Code = 99, Message = "Giao dịch không thành công, Người sử dụng hủy giao dịch" });
            return errorLocalPayments;
        }
    }
}
